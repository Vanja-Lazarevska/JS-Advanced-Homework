...
function findTheMovie(arrayOfMovies, userInput, show){
    ...
    for(let i = 0; i < arrayOfMovies.length; i++){
        if(arrayOfMovies[i].toLowerCase() == input){
            ...
            show.appendChild(movieInTheArray)
            break;
        }

        else if(userInput === ''){
            movieInTheArray.innerText = 'Please enter a movie'
            show.appendChild(movieInTheArray)
        }
...
button.addEventListener('click', function(){
    ...
    findTheMovie(movies, searchMovie, showToUser)
})