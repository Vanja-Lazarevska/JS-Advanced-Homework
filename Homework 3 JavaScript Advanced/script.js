...
let inputText = document.getElementById('elementText');
let inputColor = document.getElementById('colorInput');
let inputSize = document.getElementById('textSize');
let btn = document.getElementById('change')

let firstVariable = (element, colorInput) => element.style.color = colorInput.value

let secondVariable = (element, textSize) => textSize.value ===''? element.style.fontSize = 24 + 'px' : element.style.fontSize = textSize.value + 'px'

btn.addEventListener('click',() => {
    firstVariable(inputText, inputColor)
    secondVariable(inputText, inputSize)
})
